...
MSG1: 	db "Femto usage guide: Use ctrl-c to return to femto console, or ctrl-x if that is", 10, "intercepted by the program. Ctrl-z pauses the current program to return to the", 10, "console without lossing progress, these programs can be resumed with Ctrl-N to", 10, "resume process N. Ctrl-Space starts selecting a region, Ctrl-Shift-C copies", 10, "Ctrl-Shift-X copies then deleting the selected region, Ctrl-Shift-V pastes the", 10, "contents of the copy buffer, and Ctrl-Backspace deletes selected region. Use", 10, "Ctrl-Alt-Shift-Backspace to shutdown the computer", 10, 0
